...
#'
#' @export
get_epsilon <- function(age, eir, rho, a0, gamma_llin = 1){
  gamma_llin * eir *  (1 - rho * exp(-age / a0))
}

#' Calculate the risk of infection
#'
#' @param epsilon Vector of age-dependent EIR over time/age.
#' @param b Probability of infection upon mosquito bite.
#' @param seasonality Seasonality profile.
#' @param vaccine_efficacy Vaccine efficacy against infection (proportion)
#'
#' @export
...
#' Clinical incidence is returned from enrollment into the trial, defined by @param age_at_enrollment.
#' @param age_at_enrollment also determines the time at which the vaccine effect represented in @param vx starts.
#' The seasonality pattern in @param season needs to start at trial enrollment, and gets repeated
#' during the simulation if shorter than the simulation period defined by @param age.
#'
#' @param eir EIR per day.
#' @param age_at_enrollment Age at enrollment into trial (days).
#' @param gamma_llin Adjustment to exposure for individuals sleeping under a bednet.
#' @param vx Vaccine efficacy over time (proportion).
#' @param season Seasonality profile (vector of seasonality per day).
#' @param icm Maternal immunity level.
#' @param age Vector of age/time (in days) over a sufficiently long time, e.g. 1:(365 * 12).
#' @param n Number of biting heterogeneity groups.
